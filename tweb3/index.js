...
function decodeTxResult(result) {
  ...
  if (result[name] && result[name].data) {
    result[name].data = tryParseJson(
      switchEncoding(result[name].data, "base64", "utf8")
    );
  }
...
exports.IceTeaWeb3 = class IceTeaWeb3 {
  ...
  getTransaction(hash, options) {
    ...
    return this.rpc
      .call("tx", { hash: switchEncoding(hash, "hex", "base64"), ...options })
      .then(decodeTxResult);
  }
    ...
    if (typeof conditions === "string") {
      ...
      query = Object.keys(conditions)
        .reduce(
          ...
          [`EventNames CONTAINS '${emitter}${eventName}|'`]
        )
        .join(" AND ");
    }
  ...
  sendTransactionCommit(tx, privateKey) {
    return this.rpc
      .send("broadcast_tx_commit", signTxData(tx, privateKey))
      .then(decodeTxResult);
  }
  ...
  unsubscribe(subscriptionId) {
    ...
    return Promise.reject(
      new Error(`Error: Subscription with ID ${subscriptionId} does not exist.`)
    );
  }
